...
export const getMyNotifications = async (req: Request, res: Response) => {
  try {
    ...
    const data = await notificationService.getMyNotifications(userId, page, limit, unreadOnly);
...
export const markAllNotificationsAsRead = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ message: 'Da danh dau tat ca notification da doc', modifiedCount });
  } catch (error) {
