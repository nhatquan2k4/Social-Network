...
export const uploadMedia = async (req: Request, res: Response) => {
  try {
    ...
    const requireConversationIdForMessage = process.env.MEDIA_MESSAGE_REQUIRE_CONVERSATION_ID !== "false";
    ...
    if (purpose === "message" && requireConversationIdForMessage && !conversationId) {
      return res.status(400).json({ message: "conversationId la bat buoc voi purpose=message" });
    }
